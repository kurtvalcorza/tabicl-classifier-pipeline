...
The pipeline is not intended for decisions in health care, physical safety, criminal justice, legal rights, employment, credit, insurance, education access, or public benefits, and it has not been validated for any of them. The only validation performed is the contract and notebook conformance testing in `scripts/` and the holdout evaluation on the operator's own table; no clinical, regulatory, or independent domain validation has been carried out by the developers or by any external body, and upstream benchmark rankings are not evidence of suitability.
...
- **Supply-chain integrity:** the base checkpoint is downloaded with `hf_hub_download(..., revision=BASE_MODEL_REVISION)` at `4dcd344e…`, its SHA-256 is computed and compared with `BASE_MODEL_SHA256` (`bdc7dbd5…`), and a mismatch raises unless a checkpoint path is supplied through `DIMER_BASE_MODEL_PATH`, in which case the digest and `matches_pinned: false` are recorded in provenance rather than enforced; `tabicl` is pinned to 2.1.1.
...
- **Refusals:** fine-tuning refuses any non-CUDA device rather than silently training on CPU; the validator writes `classNames` on every result because the pipeline contract (`DIMER_CONTRACT.md`) requires it.
...
- any use that violates the BSD-3-Clause terms of the upstream `jingang/TabICL` checkpoint and `tabicl` code, or the terms of the deployment that runs the pipeline.
