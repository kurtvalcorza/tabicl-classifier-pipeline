...
`tutorials/tabiclv2_classifier_colab.ipynb` (`E2E`) and its companion `tutorials/tabiclv2_classifier_artifact_inference_colab.ipynb` (`ARTIFACT-INFERENCE`) are declared under DIMER Notebook Specification 1.1 and are **standalone** (§3.6): generated by `tools/build_notebook.py`, each carries the public reference API module `src/tabicl_classifier_pipeline/api.py` verbatim, the model identity (`jingang/TabICL` at immutable revision `4dcd344ece2c00be9e831fdd35bed57b5ad83e19`), the snapshot manifest digest (`bdc7dbd5e4ff21f8f0456fcf90c6b7cdf72dbea960f2d05b19bec19f9b3d4ed0`) and the exact `pyproject.toml` runtime pins, so the exported notebooks run without this repository or DIMER Workbench (parity enforced by `tests/test_notebook_parity.py`, `tests/test_companion_parity.py` and `tools/validate_release_assets.py`). The E2E default path loads scikit-learn's bundled Breast Cancer Wisconsin table (no download; Wine and the pinned Palmer Penguins archive are the alternatives), validates it with `validate_inputs` into an input manifest with class coverage preserved, conditions the pretrained model, evaluates it on a stratified holdout and an independent test partition against `majority_class_baseline`, LightGBM and Random Forest with strict label alignment, writes an `evaluation_report` (`sample-sanity`), optionally fine-tunes on CUDA (`RUN_FINE_TUNING`, off by default) and optionally scores an uploaded CSV with class probabilities under an explicit `argmax` rule, exports a DIMER-style serving bundle and proves it reloads from a fresh directory. The companion consumes a bundle from a separate execution (upload or `ARTIFACT_ZIP_PATH`), validates it before reconstruction, validates new unlabelled rows into an input manifest, predicts labels with class probabilities, and writes a `not-measurable` evaluation report. See `tutorials/README.md` for the registry, `docs/WEIGHTS.md` for the snapshot layout and `docs/release-verification.md` for the release gate.
